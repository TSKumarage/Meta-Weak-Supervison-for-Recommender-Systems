# -*- coding: utf-8 -*-
"""Meta Learning based User Preference estimator.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1M_MOq7WG5YCF4VLcz0eBVc3d4bMRCEeN
"""
...
#Read the data
amazon_ratings = pd.read_csv(r"drive/My Drive/amazon_reviews_us_Electronics_train.tsv",sep=",",header=0,error_bad_lines=False)
...
amazon_ratings = amazon_ratings[['userId', 'itemId', 'star_rating']]
...
class SampleGenerator(object):
    ...
    def _split_loo(self, ratings):
        ...
        for i in range(len(keys)):
          ...
          for j in range(t,len(product_ids)):
            te.append([keys[i],product_ids[j],rate[j]])
          for j in range(0,t):
            tr.append([keys[i],product_ids[j],rate[j]])
        train = pd.DataFrame(tr,columns=["userId","itemId","star_rating"])
        test = pd.DataFrame(te,columns=["userId","itemId","star_rating"])
        return train,test
    ...
    def evaluate_data(self,test_negative):
        ...
        return [torch.LongTensor(test_users), torch.LongTensor(test_items), torch.FloatTensor(ratings), torch.LongTensor(negative_users),
                torch.LongTensor(negative_items),torch.DoubleTensor(negative_ratings)]
...
class user_preference_estimator(torch.nn.Module):
    def __init__(self, config):
        ...
        self.user_e = EmbedU(802412,64)
        self.item_e = EmbedI(96799,64)
...
class MeLU(torch.nn.Module):
    ...
    def forward(self, user,item,rating, queryitem,num_local_update):
      for idx in range(num_local_update):
          if idx > 0:
              self.model.load_state_dict(self.fast_weights)
          weight_for_local_update = list(self.model.state_dict().values())
          support_set_y_pred = self.model(user,item)
      ...
      self.model.load_state_dict(self.fast_weights)
      query_set_y_pred = self.model(user,queryitem)
      self.model.load_state_dict(self.keep_weight)
      return query_set_y_pred

    def global_update(self, support_set_xs_u, support_set_xs_i, support_set_ys, query_set_xs_u, query_set_xs_i, query_set_ys,num_local_update):
        batch_sz = len(support_set_xs_i)
        losses_q = []
        for i in range(batch_sz):
            query_set_y_pred = self.forward(support_set_xs_u[i],support_set_xs_i[i], support_set_ys[i], query_set_xs_i[i],num_local_update)
...
def training(melu,user,item,rating):
  ...
  return melu.global_update(user,supp_x_i,supp_x_r,user,query_x_i,query_x_r,1)

train_loader = sample_generator.instance_a_train_loader(6,train_negatives,1024)
for batch_id,batch in enumerate(train_loader):
    user= batch[0]
    item = batch[1]
    rating = batch[2]
    # aspect_rating = batch[3]
    print(user,item)
    break
...
for epoch_number in range(0,11):
  ...
  for batch_id,batch in enumerate(train_loader):
    ...
    item = batch[1]
    rating = batch[2]
    losses.append(training(melu,user,item,rating))
    ids.append(batch_id+1)
  print("------------------------------------------------------------------------------------------------------")
  plt.plot(ids,losses)
  test_users, test_items , ratings = evaluate_data[0], evaluate_data[1] , evaluate_data[2]
  for i in range(len(test_users)):
    pred_y = melu.model(test_users[i],test_items[i])
    ratings[i] = ratings[i].reshape(1)
    s+=F.l1_loss(pred_y,ratings[i])
